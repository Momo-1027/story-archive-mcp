...
const esc=(x:unknown)=>String(x??"").replaceAll("&","&amp;").replaceAll("<","&lt;")
...
export function buildWeb(archive:Archive, adminUser?:string, adminPassword?:string) {
  ...
  const css=`
  body{font:15px/1.65 system-ui,-apple-system,sans-serif;max-width:1100px;margin:28px auto;padding:0 18px;color:#202124;background:#f7f7f8}
  nav{display:flex;gap:16px;align-items:center;margin:0 0 22px}a{color:#4f46e5;text-decoration:none}
  ...
  app.get("/",(_req,res)=>{
    ...
    res.send(layout("档案",`<h1>档案</h1>`+xs.map(s=>`
      ...
      <div class=meta>${esc(s.category)} · ${esc(s.created_at)} · #${s.id}</div>
      <p>${esc(s.summary||"")}</p>${s.tags.map((t:string)=>`<span class=tag>#${esc(t)}</span>`).join("")}</div>`).join("")));
  });

  app.get("/story/:id",(req,res)=>{
    try{
      const s:any=archive.storyOutline(Number(req.params.id));
      const all:any=archive.readAllParagraphs(s.id,true);
      res.send(layout(s.title,`<h1>${esc(s.title)}</h1><div class=meta>${esc(s.category)} · #${s.id}</div>
      <p>${esc(s.summary||"")}</p><p>${s.tags.map((t:string)=>`<span class=tag>#${esc(t)}</span>`).join("")}</p>`+
      all.paragraphs.map((p:any)=>`<div class=card id=p${p.paragraph_no}>
        <div class=meta>¶${p.paragraph_no} · paragraph_id=${p.paragraph_id}</div>
        <p>${esc(p.content).replaceAll("\n","<br>")}</p>
        ${p.comments.map((c:any)=>`<div class=comment><b>${esc(c.actor)}</b>${c.window_label?` <span class=meta>(${esc(c.window_label)})</span>`:""}：${esc(c.body)}
        <div class=meta>#${c.comment_id}${c.reply_to?` · 回复 #${c.reply_to}`:""} · ${esc(c.created_at)}</div></div>`).join("")}
      </div>`).join("")+`<form method=post action="/story/${s.id}/delete" onsubmit="return confirm('确定删除整篇？')"><button class=danger>删除整篇</button></form>`));
    }catch(e:any){res.status(404).send(layout("不存在",`<p>${esc(e.message)}</p>`));}
  });

  app.get("/upload",(_req,res)=>res.send(layout("上传",`
    <h1>上传文本</h1><form method=post action=/upload enctype=multipart/form-data>
    ...
    <label>直接粘贴正文<textarea name=content rows=18></textarea></label>
    <label>或者上传 .txt / .md 文件<input type=file name=file accept=".txt,.md,text/plain,text/markdown"></label>
    <button>上传</button></form>`)));

  app.post("/upload",upload.single("file"),(req,res)=>{
    try{
      const fromFile=req.file?.buffer.toString("utf8")||"";
      const content=String(req.body.content||"").trim() || fromFile;
    ...
    }catch(e:any){res.status(400).send(layout("上传失败",`<p>${esc(e.message)}</p><p><a href=/upload>返回</a></p>`));}
  });

  app.post("/story/:id/delete",(req,res)=>{
    archive.deleteStory(Number(req.params.id));res.redirect("/");
  });
